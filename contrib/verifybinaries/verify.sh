...
###   This script attempts to download the signature file SHA256SUMS.asc from robbiecon.org
...
WORKINGDIR="/tmp/robbiecon_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://robbiecon.org/bin/"
VERSIONPREFIX="robbiecon-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'robbiecon-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Robbiecon Core binary release signing key installed?"
   fi
