...
The RPM spec file provided here is for Robbiecon-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://robbiecon.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Robbiecon git repository](https://github.com/robbiecon/robbiecon) but are not part of
...
Currently only `Source10` is used, for the example `robbiecon.conf` file.
...
in the robbiecon git repository. At present only an SVG version of the Robbiecon
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the robbiecon
...
needed when 0.12.1 is released, a proper fix is already in the Robbiecon git
master branch.
...
idea to build Robbiecon Core against a static version of that library compiled
...
The `robbiecon.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `robbiecon-server` package. At this point, most RPM based
...
The `robbiecon-server` package should have SELinux support. How to properly do
...
Robbiecon-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/robbiecon/). He has been
packaging Robbiecon for Fedora at least since 2012.
...
4. I renamed the `robbiecon` package that contains the Qt GUI to `robbiecon-core` as
that appears to be how the general population refers to it, in contrast to
`robbiecon-xt` or `robbiecon-classic`. I wanted to make sure the general population
